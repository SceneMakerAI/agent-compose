...
### 3-1. 장면 하나가 예산보다 길면 "장면 없음" 으로 끝난다 [실측 — comp 1]
...
**실측 재현 (comp 1)**: v202 `"홈런 장면 모음"` budget=60 → `status=empty`, 0클립.
그 경기의 홈런은 scene 11 (2회 초, 역전, **74초**) 단 한 건 — 74 > 60 이라 통째로 spare 로
밀렸다. 사용자에겐 "조건에 맞는 장면 없음" 으로 보이지만 홈런은 실재했다.
이 경로가 흔한 이유: 홈런은 `FULL_CLIP_TAGS` 라 통째 클립(길다)인데, "홈런 모음" 질의는
예산을 짧게 주기 쉽다. 최소 한 건은 예산을 넘겨서라도 담거나, 초과 사실을 알려야 한다.
