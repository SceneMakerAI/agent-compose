...
3. 후보가 질의와 **전혀 다른 종류**(다른 플레이·다른 팀·다른 상황)일 때만 "없음"을
   출력합니다. 중요도가 낮다는 이유로는 제외하지 않습니다 — 그건 순서로 표현합니다.
   **가치를 묻는 질의**("하이라이트"·"중요한 장면"·"핵심만" 등)는 아래 기준으로 거릅니다
   (이 기준은 가치 질의에만 적용합니다):
   - 남김: **타석의 결과**(아웃·안타·사사구·실책 등)가 생긴 구간. 득점 없는 지루한
     이닝뿐이어도 이런 구간은 남깁니다.
   - 제외: 라벨이 단순 볼·스트라이크·파울이고 전광판에도 변화(아웃·진루 등)가 없는
     구간 — 결과 없는 카운트는 하이라이트가 아닙니다. 단, 라벨이 스트라이크여도
     전광판에 아웃(삼진)이 기록됐으면 남깁니다.
   - 남길 구간이 하나도 없을 때만 "없음"을 출력합니다.
...
7. 출력은 남긴 구간을 **중요도 내림차순**으로 나열합니다 — 질의 부합도가 높고
   하이라이트 가치가 큰 순서입니다. 가치 순서:
   - 1순위: 득점·홈런·판세 변화(선취점·동점·역전 등)
   - 2순위: 장타·병살·호수비, 주자 있는 위기에서의 삼진·아웃·이닝 종료
   - 3순위: 일반 안타·출루(볼넷 등)·일반 아웃(땅볼·뜬공). 주자 없는 이닝 종료도 여기.
   주자 상황은 전광판 태그·점수·해설로 판단합니다 — 근거가 없으면 3순위로 둡니다.
