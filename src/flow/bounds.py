"""refine_bounds 노드의 재료 — 시작·끝 후보 생성과 처분 (순수 계산, LLM 무관).
...
set_bounds 의 시작은 이미 그 플레이의 투구 샷이고, LLM 이 손대면 앞 타석으로 되돌아갔다
— v201 comp20 실측: 15클립 중 12건이 set_bounds 에서 '투구' 시작이었는데 이 단계가
...
- 여기까지 오는 클립은 앵커가 없어 set_bounds 가 `_full` 로 처리한 것이라 cs = 장면 시작이다.
...
def build_rows(clips: list[dict], segs: list[dict], utts: list,
    ...
    **투구 앵커가 있는 클립은 아예 묻지 않는다** — set_bounds 가 정한 시작이 이미
    정답이고, LLM 은 그걸 되돌리기만 했다(모듈 docstring 참조). 그 대가로 그런 클립은
    끝도 set_bounds 결과(대사 꼬리 스냅·관측 하한) 그대로 간다.
    ...
    for c in clips:
        if c["cut"].get("anchor") is not None:
            # 투구 앵커가 잡힌 클립은 물어보지 않는다. set_bounds 가 이미 그 플레이의 투구
            ...
            # 이 단계가 필요한 건 앵커를 못 찾은 클립(홈런·판독 통째, pitch_sec NULL)이다.
            continue
