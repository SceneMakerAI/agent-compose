...
- 시작 후보에는 현재 시작과의 간격이 적혀 있다("28초 앞" · "25초 뒤").
  간격이 크면 다른 타석의 투구일 수 있다 — 그 시각 해설이 이 플레이를 말하고
  있는지로 판별한다("다시 파울입니다" 처럼 앞선 투구를 말하면 버린다).
- **현재 시작이 이 플레이와 무관한 앞 타석이면 시작을 뒤로 미룬다.** 장면 경계가
  앞 타석 꼬리를 무는 경우가 있다 — 현재 시작의 해설이 다른 타자·다른 상황을
  말하고 있고 뒤쪽에 이 플레이의 투구 후보가 있으면 그쪽을 고른다.
...
def bounds_user(rows: list[dict]) -> str:
    ...
    for r in rows:
        ...
        for c in r["starts"]:
            g = c["gap"]
            where = f"{g:.0f}초 앞" if g > 0 else f"{-g:.0f}초 뒤"
            lines += _cand_lines("시작후보", c["sec"], c, note=f"{c['why']} ({where})")
        if not r["starts"]:
