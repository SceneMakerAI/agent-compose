...
- 시작 후보는 **전부 현재 시작보다 뒤**이고, 얼마나 뒤인지가 적혀 있다("25초 뒤").
  시작을 앞으로 당기는 선택지는 없다 — 고를 수 있는 건 "유지" 아니면 "미루기"다.
- **현재 시작이 이 플레이와 무관한 앞 타석이면 시작을 뒤로 미룬다.** 장면 경계가
  앞 타석 꼬리를 무는 경우가 있다 — 현재 시작의 해설이 다른 타자·다른 상황을
  말하고 있고 뒤쪽에 이 플레이의 투구 후보가 있으면 그쪽을 고른다.
- 반대로 간격이 너무 크면 **다음** 타석의 투구일 수 있다. 그 시각 해설이 이 플레이를
  말하고 있는지로 판별한다 — 이미 결과를 말했거나 다른 타자를 부르면 버리고 "유지".
